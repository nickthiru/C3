import { Stack } from "aws-cdk-lib";
const { Table, AttributeType, BillingMode } = require("aws-cdk-lib/aws-dynamodb");

class DataStack extends Stack {
  constructor(scope, id, props) {
    super(scope, id, props);

    // DDB table to store WebSocket connections. This table needs to be set/accessed
    // in the 'ConnectRoute_WebSocketLambda' (line 20).
    this.webSocketConnectionsTable = new Table(this, "WebSocketConnectionsTable", {
      partitionKey: {
        name: "connectionId",
        type: AttributeType.STRING
      },
      removalPolicy: RemovalPolicy.DESTROY,
      billingMode: BillingMode.PAY_PER_REQUEST
    });


    /*** Permissions ***/

    this.webSocketConnectionsTable.grantReadWriteData(props.connectRouteLambda);
    this.webSocketConnectionsTable.grantReadWriteData(props.disconnectRouteLambda);
    this.webSocketConnectionsTable.grantReadWriteData(props.toWebClientRouteLambda);

    // Later, need to create a DDB table for each type of user e.g. admin.
  }

}

module.exports = DataStack;